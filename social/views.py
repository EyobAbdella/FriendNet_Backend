...
from .models import ChatMessage, ChatRoom, Comment, Friend, Group, GroupMessages, Post, Like, Save, UserProfile, FriendRequest
from .serializers import( AddMembersSerializer, ChatMessageSerializer, ChatRoomSerializer, CommentSerializer, GroupMessagesSerializer, GroupSerializer,
    FriendRequestSerializer,
    LikePostSerializer,
    ListPostSerializer,
    PostSerializer,
    FriendRequestDecisionSerializer, SavePostSerializer,
...
class PeopleViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        friend_id = FriendRequest.objects.filter(Q(sender_id=user_id) | Q(receiver_id=user_id)).values_list('receiver_id', 'sender_id')
...
class UserFriendViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user_id is not None:
            friends_ids = Friend.objects.select_related('user').filter(user_id=user_id).values_list('friends', flat=True)
            return UserProfile.objects.filter(user_id__in=friends_ids).select_related('user')
...
class FriendRequestViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        user_id = self.request.user.id
        return FriendRequest.objects.filter(Q(sender_id=user_id, is_accepted=False) | Q(receiver_id=user_id, is_accepted=False))
...
class FriendRequestDecisionViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        return FriendRequest.objects.filter(receiver_id=self.request.user.id, is_accepted=False)
...
class ListPostViewSet(ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if request.user.id != post.user_id:
            return Response({"detail": "you don't have permission to update this post." }, status=status.HTTP_403_FORBIDDEN)
...
class CommentViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        return Comment.objects.filter(post_id=self.kwargs['post_pk']).select_related('user__user')
    ...
    def update(self, request, *args, **kwargs):
        ...
        if request.user.id != comment.user_id:
            return Response({"detail": "you don't have permission to update this comment."}, status=status.HTTP_403_FORBIDDEN)
        return super().update(request, *args, **kwargs)
...
class LikePostViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        return Like.objects.filter(post_id=self.kwargs['post_pk']).select_related('user__user')
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if request.user.id != like.user_id:
            return Response({"detail": "you don't have permission to delete this like."}, status=status.HTTP_403_FORBIDDEN)
        return super().destroy(request, *args, **kwargs)
...
class SavePostViewSet(ModelViewSet):
    ...
    lookup_field = 'user_id'
    def get_queryset(self):
        return Save.objects.filter(post_id=self.kwargs['post_pk']).select_related('user__user')
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if request.user.id != save.user_id:
            return Response({"detail": "you don't have permission to delete this save."}, status=status.HTTP_403_FORBIDDEN)
        return super().destroy(request, *args, **kwargs)
...
class ChatRoomViewSet(GenericViewSet, ListModelMixin, RetrieveModelMixin):
    ...
    def get_queryset(self):
            ...
            return ChatRoom.objects.filter(members=self.request.user.id).prefetch_related(
                ...
                Prefetch('message', queryset=ChatMessage.objects.select_related('sender__user')),
            )
    def get_serializer_context(self):
        return {"request": self.request}
...
class ChatMessagesViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ChatMessage.objects.filter(room_id=room_id).select_related('sender__user')
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer = ChatMessageSerializer(data=request.data, context={'user_id': user_id, "request": request, "room_id": room_id})
        serializer.is_valid(raise_exception=True)
...
class GroupViewSet(ModelViewSet):
    ...
    queryset = Group.objects.all().prefetch_related('members__user').select_related('creator__user')
    filter_backends = [DjangoFilterBackend, SearchFilter]
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.creator_id != request.user.id:
            return Response({"detail": "You don't have permission to delete the group."}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
        return super().destroy(request, *args, **kwargs)

    def update(self, request, *args, **kwargs):
        instance = self.get_object()
        if instance.creator_id != request.user.id:
            return Response({"detail": "You don't have permission to update the group."}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
        return super().update(request, *args, **kwargs)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.user_id == user_id:
            if group.creator_id == instance.user_id:
                group.delete()
                return Response({"detail": "Group Successfully deleted"}, status=status.HTTP_204_NO_CONTENT)
            group.members.remove(user_id)
            return Response({"detail": f"{instance.user} Successfully removed from group."}, status=status.HTTP_204_NO_CONTENT)
...
class GroupMessageViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        return GroupMessages.objects.filter(room_id=self.kwargs['group_pk']).select_related('room', 'sender__user')
    ...
    def create(self, request, *args, **kwargs):
        ...
        user_id = request.user.id
        serializer = GroupMessagesSerializer(data=request.data, context={'user_id': user_id, "room_id": room_id, 'request': self.request})
        serializer.is_valid(raise_exception=True)
        file_info = serializer.save()
